...
let selectCategoriesEl = document.getElementById("select-categories");
      ...
      let paymentFormEl = document.getElementById("payment-form");
      let submiteEl = document.getElementById("submit");
      let textEl = document.getElementById("outputText");
      let activities = [
      ...
      window.onload = function () {
        ...
        selectCategoriesEl.onchange = function () {
          ...
          selectActivitiesEl.options.length = 0; // reseting all option
          ...
          // bodyEl.append(actvitiesSelectEl);
        };
      };
      ...
      function populateCategoryDropdown() {
        ...
        for (let activity of activities) {
          if (activity.name === activitySelected) {
            // let option = new Option(activity.id);
            // selectActivitiesEl.appendChild(option);
            // console.log(activity);
            activityEl.innerHTML="";
            let idEl = document.createElement("p");
            idEl.textContent = activity.id;
            activityEl.appendChild(idEl);

            let descriptionEl = document.createElement("p");
            descriptionEl.textContent = activity.description;
            activityEl.appendChild(descriptionEl);

            let locationEl = document.createElement("p");
            locationEl.textContent = activity.location;
            activityEl.appendChild(locationEl);

            let priceEl = document.createElement("p");
            priceEl.textContent = activity.price;
            activityEl.appendChild(priceEl);
            
            if (activity.price > 0) {
              // paymentFormEl.innerHTML="";
              paymentFormEl.style.visibility = "visible";
              // paymentFormEl.options.length = 0;
              
            } else {
              paymentFormEl.style.visibility = "hidden";
              // paymentFormEl.options.length = 0;
            }
            ...
            // let option = new Option("");
            // paymentFormEl.appendChild(option);
    }
